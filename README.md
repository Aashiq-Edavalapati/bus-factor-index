...
| :--- | :--- |
| **Risk-Class Distribution (Donut & Ecosystem Bars)**<br>![Risk Dist](figures/eda_risk_distribution.png) | **49.7% of packages (n=220) are Abandonment-Imminent**, 19.4% (n=86) are At-Risk, and 30.9% (n=137) are Healthy. Both JavaScript and Python ecosystems show severe abandonment baselines (~49.5% and ~50.9% respectively), proving that enterprise software relies on largely unmonitored infrastructure. |
| **Inactivity & Cadence (Violin Plot & ECDF)**<br>![Inactivity](figures/eda_maintenance_inactivity.png) | Abandonment-Imminent packages exhibit a median inactivity duration exceeding 1,200 days (~3.3 years), with historical release cadences collapsing to $<1.5$ releases/year (over 80% under 1.5/yr on ECDF). In contrast, Healthy libraries maintain a median cadence of 6.2 releases/year and have pushed updates within the last 60 days. |
| **Contributor Concentration & Bus Factor Tiers**<br>![Contributor Concentration](figures/eda_contributor_concentration.png) | In Abandonment-Imminent libraries, 35.9% exhibit complete solo monopolies (BF = 1) and 16.4% rely on duopolies (BF = 2), meaning over 52% rely on $\le 2$ developers. Once lead-contributor commit share crosses **80%**, the Issue Resolution Ratio plummets from 85% down below 40%, directly precipitating maintainer burnout. |
| **Downloads vs Risk (KDE & Vanity Paradox)**<br>![Downloads](figures/eda_downloads_vs_risk.png) | Monthly download distributions overlap heavily between Abandoned and Healthy packages (both averaging $10^6$ to $10^8$ monthly downloads). Dormant libraries like *request* (15M downloads/mo) and *left-pad* (2.5M downloads/mo) continue to register millions of automated CI/CD downloads. Downloads represent **systemic exposure**, NOT maintainer health. |
| **Blast Radius (2D Centrality & Stacked Severity)**<br>![Blast Radius](figures/eda_dependents_vs_risk.png) | Over 67% of Abandonment-Imminent packages possess High (50–64) or Critical ($\ge 65$) Blast Radius scores. The 2D bubble plot maps how abandoned packages occupy high-exposure ecosystem nodes (Downloads $\ge 10^5$ and Dependents $\ge 10^2$), cascading unpatched vulnerabilities into production. |
| **Feature Correlation Matrix Heatmap**<br>![Correlation](figures/eda_feature_correlation.png) | Contributor Gini correlates strongly with Top-1 Commit Share ($r = 0.84$) and negatively with Bus Factor ($r = -0.66$) and Issue Resolution ($r = -0.32$). GitHub Stars and Downloads exhibit near-zero correlation with Inactivity Days ($r = -0.11$) and Contributor Gini ($r = -0.05$). |
